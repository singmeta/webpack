...
export class Scene2 extends Phaser.Scene {
  ...
  create() {
    room.then((room) =>
      room.onMessage((data) => {
        if (data.event === `CURRENT_PLAYERS_${room.id}`) {
          ...
          Object.keys(data.players).forEach((playerId) => {
            ...
            if (playerId !== room.sessionId && room.id === player.roomId) {
        ...
        if (data.event === `PLAYER_JOINED_${room.id}`) {
        ...
        if (data.event === "PLAYER_MOVED") {
          ...
          //console.log(data);
          //console.log(onlinePlayers[data.sessionId].map);
...
s
        ...
        if (data.event === "PLAYER_MOVEMENT_ENDED") {
          ...
          if (this.mapName === onlinePlayers[data.sessionId].map) {
            ...
            onlinePlayers[data.sessionId].stopWalking(data.position,data.charname);
          }
        }
